...
**画布是一张网格**：每一列的**中心 x** × 每一行的**中心 y** 画成淡灰**格点**——对象只停在格点上
（这是交换图的排版规范，见 [DIAGRAM_SPEC §1.1](docs/DIAGRAM_SPEC.md)；格点看得见，规范才成立）。
...
单射画**尾部钩子**（`↪`）、同构两者一起（`≅`）。用户画的映射按它**实际的**单 / 满 / 同构来
（判不出时**不猜**，退回普通箭头）；结构伴生箭头按数学身份给——`π` / `π₁` / `π₂` 是满射、`↪` 是单射、
第一同构的 `≅` 是同构。于是 `G ↠ G/N`、`im φ ↪ H`、`G/ker φ ≅ im φ` **一眼可分**。
...
![格点与吸附预览：松手落在最近的空格点](docs/assets/u10-drag.png)

_（U10：淡灰小点就是**格点**——列中心 × 行中心的交点，对象只停在这些点上。
拖动时出现的橙圈是**吸附预览**，松手就落在那里；右下角是视图工具条
（缩放百分比 · 适应窗口 · 恢复自动布局））_

![Sylow III 的图 + 格点：S₄ → 底集(Syl) → Orb(1) 同列](docs/assets/u10-grid.png)

_（U10：默认示范。格点看得见了；`Orb = Ω`（传递）补进竖直约束之后，
三层结构（群 → Ω → 轨道）排在同一列）_
...
| [docs/ROADMAP.md](docs/ROADMAP.md) | 规划：UI 重构线 U0–U11 + 里程碑 M0–M4 |
