...
- **箭头形状 = marker**：`marker-end` 挂 `-surj`（满射双箭头）、`marker-start` 挂 `-hook`（单射尾钩，要 `orient="auto-start-reverse"`）、同构两个一起；marker 必须**按视觉族成套生成**（颜色写死在定义里会出现"蓝灰线配深色箭头"）。形状由 `derive.ts` 的 `arrowOf()` 从映射属性推，判不出**不猜**。
...
U0–U5 ✅（注册表 / 输入框 / 径向菜单 / 浮层面板 / 映射构建器 / 结论层 / 交换图化）· **U6 ✅**（对象栈 · 网格化 · 箭头形状与标签摆位；**集合展开 ⏳**）· **U7 ✅** Sylow III 的图 · U8 工具条/群目录 · U9 集合构造器 · U10 拖动 · U11 宏。
