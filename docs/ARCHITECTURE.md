...
### 3.6 存在层级：值「在图里处于什么位置」（U6 落地）

`ValueType` 回答"数据长什么样"；**`ValueSort` 回答"它去哪"**——两者正交，合成一行：
...
从前 `canvasShape()` 各自 switch 一遍，把 `elements` 与 `subgroups` 都判成 `'set'`，
于是画布上分不出"对象"与"表格"——这是本轮的出发点。

**判据**：能作为某个映射的源或靶的，才配当顶点。
完整论证见 [DIAGRAM_SPEC.md](DIAGRAM_SPEC.md) §3 / §5 / §6。

**配套操作 `底集(S)`**：把 `list` 提升成 `set`。数学上是"忘记结构取底集"，
作用是**让列表能继续参与运算**：

```
Syl_p(G)（列表，不上画布）──底集──▶ Ω（集合，上画布）──被群作用──▶ 轨道 / 稳定子
```
...
| 取底集 | `底集(S)` | 原子构造（忘记结构）| 本地（→ `set` 值，可被作用）|
