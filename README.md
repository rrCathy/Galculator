...
**画布铺满窗口，面板浮在它上面**：左上并排三个可收起的抽屉——「对象」（含上面那两块输入框）/「操作」（运算产生的对象）/「信息」，
左下是「数值」上拉抽屉。
...
| 按钮 | 行为 |
|---|---|
| 基本 / 元素 / 子群 | 切左上的**信息面板**（「元素」是横滚表格：**列 = 元素、行 = 属性**）|
...
- **U2.5 已完成（UI v3）**：面板从侧栏改成**浮层抽屉**（左上三抽屉 + 左下数值上拉），画布铺满并给抽屉让位；
  节点左上角**深色悬浮球**（4 按钮：基本 / 元素 / 子群 / 操作）；**多对象球**挂在显示区正上方；
  「元素」是**横滚表格**（列 = 元素、行 = 属性），格子**可拖进数值区**；**KaTeX 接上**（面板里的 TeX 真渲染）。
...
![信息面板的「元素」tab：列 = 元素、行 = 属性](docs/assets/u3-elements.png)

_（元素横滚表格：阶 / ∈Z? / 共轭类 / 类大小 / 中心化子；行首列粘住，格子可直接拖进左下数值区）_
