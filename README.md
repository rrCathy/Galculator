...
**所有操作都来自一张注册表**（[`src/gal/ops.ts`](apps/web/src/gal/ops.ts)，27 条，按机制分组），每条声明**命名参数 + 类型**；
`opsFor(selection)` 据此派生出"选中什么能做什么"，左栏「可用操作」面板可点选插入（选中画布对象后自动收敛到那 N 条）：
...
apps/web/            Vite + React 19 + TS 前端
  src/gal/
    ...
    derive.ts        对象表 → 画布图
  src/ui/            左栏三区 + 操作面板（按选中过滤）· 两块输入框（实时校验）· SVG 交换图画布 · 详情条
...
- 验证：`tsc` 零错误 · U0 **52/52** + U1 **35/35** 断言 · 真浏览器控制台零错误。
- **规划已收敛**（[INTERACTION](docs/INTERACTION.md) v2 + [ROADMAP](docs/ROADMAP.md) 的 U0–U7）：5 条决策已定。
- 下一步 **U2**：画布交互状态机 + 径向菜单（看 / 算 / 造三类）+ 左栏详情态——**在对象旁边就能操作**。
...
_（U1 截图：名字栏 `placeholder` 显示将分配的名字 `B`；下方提示已避开 83 个调用名）_
